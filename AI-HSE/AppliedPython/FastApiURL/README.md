...
- ✅ Получение оригинального URL по короткому коду (JSON-ответ)
...
Возвращает JSON с оригинальным URL и служебным сообщением.
...
# 2. Получить оригинальный URL по короткому коду
curl http://localhost:9999/mylink
# Ответ содержит поле original_url. Откройте этот URL в браузере.
...
Примечание по Swagger UI и CORS:
- Эндпоинт `/{short_code}` возвращает JSON с `original_url` вместо прямого HTTP-редиректа, чтобы избежать CORS-проблем в браузере и Swagger UI. Используйте полученный `original_url` для перехода в браузере.
...
#### Временные зоны и истечение срока
- Поле `expires_at` обрабатывается в UTC. Передавайте время в формате UTC (например, `2026-12-31T23:59:00Z`) или указывайте таймзону (например, `2026-12-31T23:59:00+03:00`).
- Внутри приложения время приводится к наивному UTC для корректных сравнений, а просроченные ссылки автоматически удаляются.
