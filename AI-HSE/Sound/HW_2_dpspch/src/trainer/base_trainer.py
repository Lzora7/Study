...
class BaseTrainer:
    ...
    def _train_epoch(self, epoch):
        ...
        self.is_train = True
        self.model.train()
        # Set batch transforms to training mode (for SpecAugment, etc.)
        if self.batch_transforms is not None:
            train_transforms = self.batch_transforms.get("train")
            if train_transforms is not None:
                for transform in train_transforms.values():
                    if hasattr(transform, 'train'):
                        transform.train()
        self.train_metrics.reset()
        # Set step to epoch for epoch-level metrics
        ...
        for batch_idx, batch in enumerate(
            ...
            try:
                ...
                if self.skip_oom and ("out of memory" in str(e).lower() or "OOM" in str(e)):
                    self.logger.warning(f"OOM on batch. Skipping batch. Error: {e}")
                    if self.device == "cuda" and torch.cuda.is_available():
                        torch.cuda.empty_cache()  # free some memory for CUDA
                    elif self.device == "mps" and torch.backends.mps.is_available():
                        if hasattr(torch.mps, 'empty_cache'):
                            torch.mps.empty_cache()  # free some memory for MPS
                    continue
                else:
                    raise e
            ...
            if batch_idx % self.log_step == 0:
                ...
                self.writer.add_scalar("step", step)
                self.writer.add_scalar("epoch", epoch)
                self.logger.debug(
    ...
    def transform_batch(self, batch):
        ...
        if transforms is not None:
            for transform_name in transforms.keys():
                ...
                # Skip None values (e.g., inference transforms set to null)
    ...
    def _get_grad_norm(self, norm_type=2):
        ...
        Returns:
            total_norm (float): the calculated norm. Returns 0.0 if no gradients.
        """
        ...
        # Если нет градиентов (например, после пропуска батча с NaN loss)
        if len(parameters) == 0:
            return 0.0
