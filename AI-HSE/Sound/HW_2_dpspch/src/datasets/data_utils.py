    ...
    for transform_type in batch_transforms.keys():
        ...
        if transforms is not None:
            for transform_name in transforms.keys():
                ...
                # Skip None values (inference transforms set to null)
...
def get_dataloaders(config, text_encoder, device):
    ...
    for dataset_partition in config.datasets.keys():
        ...
        # Set training mode for instance transforms
        if hasattr(dataset, 'instance_transforms') and dataset.instance_transforms is not None:
            for transform_name, transform in dataset.instance_transforms.items():
                if transform_name == "get_spectrogram":
                    continue  # Skip special key
                if transform is not None:
                    if dataset_partition == "train":
                        # Set training mode for train dataset
                        if hasattr(transform, 'train'):
                            transform.train()
                        # Handle Compose (contains list of transforms)
                        elif hasattr(transform, 'transforms'):
                            for t in transform.transforms:
                                if hasattr(t, 'train'):
                                    t.train()
                    else:
                        # Set eval mode for val/test datasets
